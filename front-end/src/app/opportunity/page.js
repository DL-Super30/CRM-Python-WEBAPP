...
const Dashboard = () => {
  ...
  const groupForKanban = (data) => {
    return [
      { id: '1', title: 'Visiting', color: 'bg-[#FFCCCC]', leads: data.filter(lead => lead.lead_status === 'Not Contacted') },
      { id: '2', title: 'Visited', color: 'bg-[#FFFF99]', leads: data.filter(lead => lead.lead_status === 'Attempted') },
      { id: '3', title: 'Demo attended', color: 'bg-[#CCFFCC]', leads: data.filter(lead => lead.lead_status === 'Opportunity') },
      { id: '4', title: 'Lost Opportunity', color: 'bg-[#CCCCFF]', leads: data.filter(lead => lead.lead_status === 'Cold Lead') },
    ];
  };
                ...
                {kanbanColumns.map((column) => (
                  <div key={column.id} className={`flex flex-col w-1/4 p-4 border rounded-lg ${column.color}`}>
                    ...
                    {column.leads.length > 0 ? (
                      ...
                      <p className="text-gray-500">Opportunity Data Not Available</p>
                    )}
                  </div>
                ))}
              </div>
