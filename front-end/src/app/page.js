            ...
            try {
                ...
                if (error.response) {
                    ...
                    if (status === 422) {
                        ...
                        setLoginError(`Validation Errors: ${Array.isArray(details) ? details.join(', ') : 'An unknown validation error occurred.'}`);
                    } else if (status === 404) {
                        setLoginError('The endpoint is not found. Please check the API URL.');
                    } else if (status === 401) {
                        setLoginError('Invalid email or password');
                    } else {
                        setLoginError(`Error ${status}: ${data.message || 'An error occurred. Please try again.'}`);
                    }
                } else {
