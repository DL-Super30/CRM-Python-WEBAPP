...
export default function Lead() {
  ...
  const fetchLeads = async () => {
    try {
      const response = await axios.get('http://18.216.178.154:8000/getleads');
  ...
  const handleDeleteClick = async (leadToDelete) => {
    if (window.confirm('Are you sure you want to delete this lead?')) {
      try {
        await axios.delete(`http://18.216.178.154:8000/deletelead/${leadToDelete.id}`);
    ...
    if (validateForm()) {
      try {
        ...
        if (selectedLead) {
          ...
          await axios.put(`http://18.216.178.154:8000/updatelead/${selectedLead.id}`, formValues);
          window.alert('Lead details Updated Successfully');
        } else {
         
          await axios.post(`http://18.216.178.154:8000/createleads`, formValues);
  ...
  return (
    <main>
      ...
      <button onClick={handleDleteSelectedLead}>delete</button>
