...
export default function Lead() {
  ...
  const handleDeleteClick = async (leadToDelete) => {
    if (window.confirm("Are you sure you want to delete this lead?")) {
      try {
        await axios.delete(
          `http://18.234.102.235:8000/deletelead/${leadToDelete.id}`
        );
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        if (selectedLead) {
          await axios.put(
            `http://18.234.102.235:8000/updatelead/${selectedLead.id}`,
            formValues
          );
          window.alert("Lead details Updated Successfully");
        } else {
          await axios.post(
            `http://18.234.102.235:8000/createleads`,
            formValues
          );
      ...
      <div className="container-lg border shadow-lg mt-5 p-5">
        <div className="flex flex-col lg:flex-row lg:justify-between">
          <div className="flex flex-col lg:flex-row lg:items-center gap-2 mb-4 lg:mb-0">
            ...
            <select className="text-2xl text-blue-400 border rounded">
              ...
              <option value="hari" className="text-lg text-black">
                Hari
              </option>
              <option value="Krishna" className="text-lg text-black">
                Krishna
              </option>
              <option value="Jayanth" className="text-lg text-black">
                Jayanth
              </option>
            </select>
          </div>
        ...
        <div className="flex flex-col lg:flex-row items-center gap-2 mt-4">
          ...
          <div className="flex gap-5 border rounded-xl">
            <button
              className={`hover:bg-blue-500 hover:text-white text-xl border-none rounded-xl p-2 ${
            ...
            <form className="space-y-4" onSubmit={handleFormSubmit}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  />
                  {formErrors.cc && (
                    <p className="text-red-500 text-xs mt-1">{formErrors.cc}</p>
                  )}
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">
                ...
                <div>
                  ...
                  >
                    <option value="" disabled selected></option>
                    <option value="Hyd Offline Campus">
                      {" "}
                      Hyd Offline Campus
                    </option>
                    <option value="BLR Offline Campus">
                      BLR Offline Campus
                    </option>
                    {/* <option value="option3">Option 3</option> */}
                  </select>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={handleInputChange}
                    className=" block w-full border-b border-gray-300 bg-transparent rounded-none  focus:outline-none p-2"
                    rows="1"
                  ></textarea>
                </div>
              </div>
                  ...
                  <tr key={lead.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <button
                        ...
                        className="text-green-500 hover:text-pink-700 ml-4"
                      >
                        View
                      </button>
                    </td>
